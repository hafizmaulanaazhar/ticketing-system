    ...
    <div class="bg-gradient-to-r from-red-100 to-red-50 rounded-lg shadow-md p-6 flex flex-col justify-between">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold text-red-600">{{ auth()->user()->tickets()->where('status', 'open')->count() }}</p>
            </div>
    ...
    <div class="bg-gradient-to-r from-green-100 to-green-50 rounded-lg shadow-md p-6 flex flex-col justify-between">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold text-green-600">{{ auth()->user()->tickets()->where('status', 'close')->count() }}</p>
            </div>
                    ...
                    <td class="px-4 py-2">
                        <span class="px-2 py-1 text-xs font-semibold rounded-full 
                            {{ $ticket->status === 'open' ? 'bg-red-100 text-red-800' : 
                               ($ticket->status === 'progress' ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800') }}">
